...
- **Source fallback** — `applySafetyNetSections` adds at most three descriptive message links only when the recap contains no known source link. Empty sections and image inventories are omitted. Actual included message counts and date spans appear near the summary title.
- **Viewing-channel context** — Slack's new agent experience stopped delivering `assistant_thread_context_changed` to `assistant_view` apps, so stored context only reflects the channel in view at thread start. `appContextChannelId` (handlers/assistant.ts) already reads the newer per-message `app_context` field as a fallback for an empty source; saved sources stay pinned to their thread. It stays null until the app migrates to `agent_view` (irreversible — see docs/slack_configuration.md before attempting).
