use tldr::response::{create_replace_original_payload, create_ephemeral_payload};
...
fn test_replace_original_payload_with_text() {
    ...
    assert!(payload_str.contains("\"replace_original\":true"), 
        "Payload should include replace_original field");
    assert!(payload_str.contains("\"text\":\"Test message\""), 
...
fn test_replace_original_payload_hide_command() {
    ...
    assert!(payload_str.contains("\"replace_original\":true"), 
        "Payload should include replace_original field");
    assert!(payload_str.contains("\"text\":\" \""), 
...
fn test_ephemeral_payload() {
    ...
    // Verify payload structure
    assert!(payload_str.contains("\"response_type\":\"ephemeral\""), 
        "Payload should include ephemeral response_type");
    assert!(payload_str.contains("\"text\":\"Test ephemeral message\""), 
        "Payload should include the text field with correct content");
}
...
fn test_worker_response_format_consistency() {
    ...
    assert!(payload_str.contains("\"text\":\"Processing your request...\""), 
