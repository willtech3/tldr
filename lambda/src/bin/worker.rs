...
pub async fn function_handler(event: LambdaEvent<Value>) -> Result<(), Error> {
    ...
    match summarize::summarize(&mut slack_bot, &config, &task).await {
        Ok(Some(summary)) => {
            deliver::deliver(&slack_bot, &http_client, &task, Some(summary))
                .await
                .map_err(|e| Error::from(format!("Processing error: {e}")))?;
        }
        Ok(None) => {
            deliver::deliver(&slack_bot, &http_client, &task, None)
                .await
                .map_err(|e| Error::from(format!("Processing error: {e}")))?;
        }
        Err(e) => {
            ...
            deliver::deliver_error(&slack_bot, &http_client, &task, error_message)
                .await
                .map_err(|e| Error::from(format!("Processing error: {e}")))?;
        }
    }
